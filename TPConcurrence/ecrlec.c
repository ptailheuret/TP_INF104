...
int main(int argc, char *argv[]) {
   ...
   * Init(verrou_antifamine, compteur)
   */
  ...
  if ((verrou_antifamine = sem_open(VERROUANTIFAMINE, O_CREAT, 0644, compteur)) == SEM_FAILED) {
    perror("init verrou Antifamine");
    exit(1);
  }
  ...
  sem_close(verrou_antifamine);
  sem_unlink("VERROUANTIFAMINE");
...
void lecteur(int le_lecteur){
  ...
  if (premier(le_lecteur)) {
    printf("========================= Lecteur %d est le premier -> sem_wait  \n", le_lecteur);
    sem_wait(verrou);
  }
  ...
  if (dernier(le_lecteur)) {
    printf("========================= Lecteur %d est le dernier -> sem_post  \n", le_lecteur);
